...
<body>  
<%
	Teacher user = (Teacher) SchoolApp.userDatabase.get(Integer.parseInt(request.getParameter("id")));
	session.setAttribute("user", user);
%>
<form method="post" name="form" action="UpdateUsers.jsp">  
  <div class="container">  
    <h1> Teacher Registration Form</h1>  
  <hr> 
<label> First Name </label> 
<input type="text" name="firstname" placeholder= "Enter Firstname" value="<%= user.getFirstName() %>" size="15" required />
<label> Last Name </label>    
<input type="text" name="lastname" placeholder="Enter Lastname" value="<%= user.getLastName() %>" size="15"required />   
<label> Middle Name </label>   
<input type="text" name="middlename" placeholder="Enter Middlename" value="<%= user.getMiddleName() %>" size="15"  />    
...
String dob = date.toString().substring(8, 10)+"-"+date.toString().substring(4, 7)+"-"+date.toString().substring(24, 28);%>
 <input type="text" name="dob" placeholder="DD-MMM-YYYY" value="<%= dob%>" size="15" required/>  <br>

<label for="email">Email </label>  
 <input type="text" placeholder="Enter Email ID" value="<%= user.getEmail() %>" name="email" required> 
...
<input type="text" name="phone" placeholder="Enter 10 digit phone number" value="<%= user.getPhoneNo() %>" size="10" required>   
<div>  
...
<textarea name="address" cols="80" rows="3" placeholder="Enter Current Address" required="required"><%out.print(user.getAddress()); %></textarea>  <br> 

<label> Subject </label>    
<input type="text" name="subject" placeholder="Enter the Subject" value="<%= user.getSubject() %>" size="15"required /> <br>
